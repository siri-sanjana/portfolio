...
class x {
  ...
  #u(e) {
    this.#s = true; // Always true to force rendering
    this.#w();
  }
  #v() {
    // Ignore visibility change to keep rendering
  }
  #f() {
...
function TouchStart(e) {
  if (e.touches.length > 0) {
    // We do NOT prevent default here so that scrolling works!
    // e.preventDefault(); 
...
export function initBallpit(canvasId, options = {}) {
  const e = document.getElementById(canvasId);
  if (!e) {
  ...
  // We do NOT want to block scrolling, so we remove touch-action: none from the canvas style.
  // The React bits had e.style.touchAction = 'none' which blocks locomotive scroll!
